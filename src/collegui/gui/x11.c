...
CGUIState *CCreateGUI(int x, int y, unsigned int width, unsigned int height,
  ...
  Window window =
      XCreateSimpleWindow(display, root, x, y, width, height, 0, 0, 0xffffffff);
  ...
  switch (ev->type) {
    case Expose:
      for (int i = 0; i < state->button_count; i++) {
        ...
        XDrawRectangle(state->display, state->window, state->gc, btn->x, btn->y,
                       btn->width, btn->height);
        XDrawString(state->display, state->window, state->gc, btn->x + 5,
                    btn->y + 15, btn->label, strlen(btn->label));
      }
      break;
        ...
        case Button1:
          ...
          for (int i = 0; i < state->button_count; i++) {
            ...
            if (x >= b->x && x <= b->x + b->width && y >= b->y &&
                y <= b->y + b->height) {
              if (b->callback) b->callback(b->user_data);
            }
          }
          break;
        case Button2:
...
bool CShowButton(CGUIState *state, int x, int y, int width, int height,
  ...
  state->buttons[state->button_count++] = (Button){.x = x,
                                                   .y = y,
                                                   .width = width,
                                                   .height = height,
                                                   .label = label,
