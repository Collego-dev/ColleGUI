...
static LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam,
  ...
  switch (msg) {
    case WM_COMMAND: {
      ...
      if (ctrl) {
        CallbackData *cb =
            (CallbackData *)GetWindowLongPtr(ctrl, GWLP_USERDATA);
        if (cb && cb->callback) {
          ...
          if (_wcsicmp(className, L"BUTTON") == 0) {
            ...
            if (style & BS_AUTOCHECKBOX) {
              BOOL checked =
                  SendMessageW(ctrl, BM_GETCHECK, 0, 0) == BST_CHECKED;
              ((void (*)(bool, void *))cb->callback)((checked ? true : false),
                                                     cb->user_data);
            } else {
...
CGUIState *CCreateGUI(int x, int y, unsigned int width, unsigned int height,
  ...
  HWND hwnd = CreateWindowA(title, title, WS_OVERLAPPEDWINDOW, x, y, width,
...
bool CShowButton(CGUIState *state, int x, int y, int width, int height,
                 const char *label, void (*callback)(void *), void *user_data) {
  wchar_t wlabel[256];
  MultiByteToWideChar(CP_UTF8, 0, label, -1, wlabel,
                      sizeof(wlabel) / sizeof(wlabel[0]));
  HWND btn = CreateWindowW(
      L"BUTTON", wlabel, WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON | WS_TABSTOP,
...
bool CShowLabel(CGUIState *state, int x, int y, int width, int height,
                const char *text) {
  wchar_t wtext[256];
  MultiByteToWideChar(CP_UTF8, 0, text, -1, wtext, 255);
  wtext[255] = L'\0';
  HWND label = CreateWindowW(
      L"STATIC", wtext, WS_CHILD | WS_VISIBLE | SS_CENTER | WS_TABSTOP, x, y,
      width, height, state->hwnd, NULL, GetModuleHandleW(NULL), NULL);
  return label != NULL;
}

bool CShowCheckBox(CGUIState *state, int x, int y, const char *label,
                   bool checked, void (*callback)(bool, void *),
                   void *user_data) {
  wchar_t wlabel[256];
  MultiByteToWideChar(CP_UTF8, 0, label, -1, wlabel, 255);
  wlabel[255] = L'\0';
  HWND chk = CreateWindowW(
      L"BUTTON", wlabel, WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX | WS_TABSTOP,
...
bool CShowListBox(CGUIState *state, int x, int y, int width, int height,
                  const char **items, int item_count,
                  void (*callback)(int, void *), void *user_data) {
  HWND list = CreateWindowW(
      L"LISTBOX", NULL,
      WS_CHILD | WS_VISIBLE | WS_BORDER | WS_TABSTOP | LBS_STANDARD, x, y,
      width, height, state->hwnd, NULL, GetModuleHandleW(NULL), NULL);
...
bool CShowTextBox(CGUIState *state, int x, int y, int width, int height,
...
bool CSetFont(HWND hwnd, const char *font_name, int font_size, bool bold) {
  wchar_t wfont[128];
  MultiByteToWideChar(CP_UTF8, 0, font_name, -1, wfont,
                      sizeof(wfont) / sizeof(wfont[0]));
  HFONT hFont = CreateFontW(
      -MulDiv(font_size, GetDeviceCaps(GetDC(hwnd), LOGPIXELSY), 72),  // 高さ
      0, 0, 0, bold ? FW_BOLD : FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
